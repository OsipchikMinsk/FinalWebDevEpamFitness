...
public class EnglishLang implements Command {
    private static final Logger logger = LogManager.getLogger(EnglishLang.class);
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, DaoException, ServiceException, CommandException {
        ...
        session.setAttribute(SessionAttributeName.LOCALE, Locale.ENGLISH);
        try {
